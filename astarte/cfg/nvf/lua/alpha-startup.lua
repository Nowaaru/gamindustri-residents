...
do
	...
	function http.encode(str)
		return (
			str:gsub(".", function(item_to_replace)
	...
	local client = setmetatable({}, {
		__call = function(self, api_key)
			...
			function object:getFiles()
				return http.fetch(
					([[%s/get_files/search_files?tags=%s&Hydrus-Client-API-Access-Key=%s]]):format(
						self.HYDRUS_URL,
						http.encode('["system:filetype = image/jpg, image/png, apng"]'),
						self.API_KEY
					)
				)
			...
			function object:getRender(fileId, width, height)
				return ("%s/get_files/render?file_id=%i&download=true&render_quality=0&render_format=2%s&%s&Hydrus-Client-API-Access-Key=%s"):format(
					self.HYDRUS_URL,
					fileId,
					width and ("&width=" .. tostring(width)) or "",
					height and ("&height=" .. tostring(height)) or "",
					self.API_KEY
				)
			end
...
local IMAGE_SOURCES = {
	{ "/home/astarte/Pictures/IMG_1883.png", 0.5 },
}
...
local FOOTERS = {
	...
	"good luck.",
	(
		(DATE.month == 12 or DATE.month == 1) and ""
		or (
			DATE.month == 11 and ""
			or (
				DATE.month == 10 and "󰮣"
				or (
					(DATE.month == 7 or DATE.month == 6 or (DATE.month == 5 and DATE.day >= 19)) and "󰖨"
					or (
						(DATE.month == 5 or DATE.month == 4 or DATE.month == 3) and ""
						or (DATE.month == 2 and "♥" or "")
					)
				)
			)
		)
...
local function weighted_random(LIST)
	...
	for i = 1, #tableIndices do
		...
		if weightsCursor >= randomValue then
			return (
				currentIndex.src
				or currentIndex.item
				or currentIndex.img
				or currentIndex.image
				or currentIndex.source
				or currentIndex[1]
			),
				weightsCursor,
				randomValue
